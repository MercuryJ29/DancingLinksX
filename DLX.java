...
public class DLX {
	...
	/*
	 * #输入图函数#
	 * 先将图的信息输入到一个二维数组中，再根据这个二维数组来构造符合算法的图结构
	 * */
	 ...
	 * ②将该编号入栈   
	 * ③修改该行结点对应的列结点的count值即count--
	 * ④将该行的行标对应的列节点的count值设置为-1，表示已经将该行隐藏
	 * */
	public void hideRow(int row_number) {
		...
		//System.out.println("已隐藏第"+row_number+"行");
	...
	public void execute() {
		...
		pre=-1;						//从编号为0的结点开始遍历
		while(stack.top!=-1) {			//遍历完毕，说明最上一层已经遍历完了
